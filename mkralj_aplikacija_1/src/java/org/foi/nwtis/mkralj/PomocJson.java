...
public class PomocJson
{
    ...
    public List<String> dajCistiTekstLista(String attributeName)
    {
        Gson gson = new Gson();
        JsonArray jArray;
        try
        {
            jArray = gson.fromJson(this.jsonText, JsonArray.class);
        } catch (Exception ex)
        {
            return null;
        }
        ...
        for (JsonElement jElement : jArray)
        {
            JsonObject jO = gson.fromJson(jElement, JsonObject.class);
            try
            {
                returnList.add(jO.get(attributeName).getAsString());
            } catch (Exception ex)
            {
